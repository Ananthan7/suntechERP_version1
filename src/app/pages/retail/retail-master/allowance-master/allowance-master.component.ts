...
export class AllowanceMasterComponent implements OnInit {
  ...
  @ViewChild("overlayreport_headingSearch")
  overlayreport_headingSearch!: MasterSearchComponent;
  ...
  data: any;
  calcBasisOptions = [
    { value: "0", label: "Basic Salary" },
    { value: "1", label: "Earned Basic Salary" },
  ];
  ...
  ngOnInit(): void {
    ...
    if (this.content?.FLAG) {
      if (this.content?.FLAG == "VIEW") {
        this.viewMode = true;
        this.allowanceMasterForm.controls.value.setValue(
          this.commonService.decimalQuantityFormat(
            this.content.ALLMST_AMOUNT,
            "AMOUNT"
          )
        );
        this.allowanceMasterForm.controls["value"].disable();
  ...
  setFormValues() {
    ...
    let api =
      "AllowanceMaster/GetAllowanceMasterWithCode/" + this.content.ALLMST_CODE;
    console.log(api);
    let Sub: Subscription = this.dataService
      ...
      .subscribe((result: any) => {
        ...
        this.allowanceMasterForm.controls.credit_to_employeeAC.setValue(
          this.data.ALLMST_CREDIT_EMPAC
        );
    ...
    this.allowanceMasterForm.controls.description.setValue(
      this.content.ALLMST_DESC
    );
    this.allowanceMasterForm.controls.glcode.setValue(
      this.content.ALLMST_ACCODE
    );
    this.allowanceMasterForm.controls.value.setValue(
      this.commonService.decimalQuantityFormat(
        this.content.ALLMST_AMOUNT,
    ...
    this.allowanceMasterForm.controls.calc_method.setValue(
      this.content.ALLMST_PERCENFIXED.toString()
    );
    this.allowanceMasterForm.controls.avoid_fraction.setValue(
      this.content.ALLMST_AVOIDFRACTION
    );
    this.allowanceMasterForm.controls.calc_basis.setValue(
      this.content.ALLMST_BASIS.toString()
    );

    this.allowanceMasterForm.controls.report_heading.setValue(
      this.content.ALLMST_REPORTHEADINGTO
    );
    this.allowanceMasterForm.controls.period.setValue(
      this.content.ALLMST_YEARMONTHLY
    );
    this.allowanceMasterForm.controls.leave_salary.setValue(
      this.content.ALLMST_LS
    );
    this.allowanceMasterForm.controls.countrycode.setValue(
      this.content.ALLMSTCOUNTRYCODE
    );
    ...
    this.allowanceMasterForm.controls.userdefined10.setValue(
      this.content.UDF10
    );
    this.allowanceMasterForm.controls.userdefined11.setValue(
      this.content.UDF11
    );
    this.allowanceMasterForm.controls.userdefined12.setValue(
      this.content.UDF12
    );
    this.allowanceMasterForm.controls.userdefined13.setValue(
      this.content.UDF13
    );
    this.allowanceMasterForm.controls.userdefined14.setValue(
      this.content.UDF14
    );
    this.allowanceMasterForm.controls.userdefined15.setValue(
      this.content.UDF15
    );
  }
  ...
  submitValidations(form: any) {
    if (this.commonService.nullToString(form.code) == "") {
      ...
      return true;
    } else if (this.commonService.nullToString(form.calc_basis) == "") {
  ...
  postData() {
    return {
      ...
      ALLMST_BASIS: this.allowanceMasterForm.value.calc_basis || 0,
  ...
  formSubmit() {
    ...
    if (this.submitValidations(this.allowanceMasterForm.value)) return;
  ...
  update() {
    let API =
      "AllowanceMaster/UpdateAllowanceMaster/" +
      this.allowanceMasterForm.value.code;
    let postData = this.postData();
    let Sub: Subscription = this.dataService
  ...
  deleteallowance() {
    ...
    this.showConfirmationDialog().then((result) => {
      if (result.isConfirmed) {
        let API =
          "AllowanceMaster/DeleteAllowanceMaster/" +
          this.allowanceMasterForm.value.code;
        let Sub: Subscription = this.dataService
  ...
  onInputChange(event: Event): void {
    ...
    input.value = value;
    this.allowanceMasterForm.controls["value"].setValue(value, {
      emitEvent: false,
    });
  }
