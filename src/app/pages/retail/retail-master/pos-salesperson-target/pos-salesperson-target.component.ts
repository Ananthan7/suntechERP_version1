...
export class PosSalespersonTargetComponent implements OnInit {
  ...
  salesmanSelected(event: any) {
    this.salespersontargetform.controls.salesman.setValue(event.SALESPERSON_CODE)
  }
  ...
  getmetal_divisionvalues(){
    ...
    let Sub: Subscription = this.apiService.getDynamicAPI(API)
      .subscribe((result: any) => {
        ...
        const diaDivisionControl = this.salespersontargetform?.get('dia_division');
  ...
  getdiamond_divisionvalues(){
    ...
    let Sub: Subscription = this.apiService.getDynamicAPI(API)
      .subscribe((result: any) => {
        ...
        const diaDivisionControl = this.salespersontargetform?.get('metal_divisions_');
        if (diaDivisionControl) {
          if(this.flag == undefined){
            diaDivisionControl.setValue(allDivisionCodes); 
          }
  ...
  ViewController(DATA: any) {
    ...
    this.salespersontargetform.controls.code.setValue(this.content?.TARGET_CODE);
    ...
    const formattedDate2 = new Date(+dateParts2[2], +dateParts2[1] - 1, +dateParts2[0]);
    ...
    this.salespersontargetform.controls.salesman.setValue(this.content?.SALESPERSON_CODE);
    this.salespersontargetform.controls.finyear.setValue(this.content?.FYEARCODE);
    this.salespersontargetform.controls.diamond_division.setValue(this.content?.DIA_DIVISIONS);
    this.salespersontargetform.controls.metal_division.setValue(this.content?.MTL_DIVISIONS);
    this.salespersontargetform.controls.goldquantity.setValue(this.commonService.decimalQuantityFormat(
      this.commonService.emptyToZero(this.content?.GOLD_QTY),"METAL"));
    this.salespersontargetform.controls.goldmakingcharge.setValue(this.commonService.decimalQuantityFormat(
      this.commonService.emptyToZero(this.content?.GOLD_AMOUNT),"AMOUNT"));
    this.salespersontargetform.controls.diamondjewellery.setValue(this.commonService.decimalQuantityFormat(
      this.commonService.emptyToZero(this.content?.DIA_AMOUNT),"AMOUNT"));
  }
  ...
  detailsapi(fm_id: any) {
    ...
    let Sub: Subscription = this.apiService.getDynamicAPI(API)
      .subscribe((result: any) => {
        ...
        this.salespersontargetform.controls.code.setValue(this.dyndatas?.TARGET_CODE);
        if (this.dyndatas?.DIA_DIVISIONS) {
          this.salespersontargetform.controls.dia_division.setValue(this.dyndatas.DIA_DIVISIONS.split(','));
      }
      
      if (this.dyndatas?.MTL_DIVISIONS) {
          this.salespersontargetform.controls.metal_divisions_.setValue(this.dyndatas.MTL_DIVISIONS.split(','));
      }

      if(this.dyndatas?.METAL_QTY == 'F'){
        this.salespersontargetform.controls.diamond_division.setValue(this.dyndatas.METAL_QTY);
      }else if(this.dyndatas?.METAL_MKGCHARGE == 'P'){
        this.salespersontargetform.controls.diamond_division.setValue(this.dyndatas.METAL_MKGCHARGE);
      }

      if(this.dyndatas?.PROFIT == 'P'){
        this.salespersontargetform.controls.metal_division.setValue(this.dyndatas.PROFIT);
      }else if(this.dyndatas?.SALES_AMOUNT == 'F'){
        this.salespersontargetform.controls.metal_division.setValue(this.dyndatas.SALES_AMOUNT);
      }
      ...
      this.commonService.emptyToZero(split_value),"METAL");
      split_gold = this.commonService.decimalQuantityFormat(
        this.commonService.emptyToZero(split_gold),"AMOUNT");
  ...
  formSubmit() {
    ...
    const postData = {
      ...
      "DIA_DIVISIONS": this.salespersontargetform.controls.dia_division.value.join(','),
      "MTL_DIVISIONS": this.salespersontargetform.controls.metal_divisions_.value.join(','),
      ...
      "SALES_AMOUNT": this.salespersontargetform.controls.diamond_division.value == "F" ? 'F' : "",   //"s",
      "METAL_MKGCHARGE": this.salespersontargetform.controls.metal_division.value == "P" ? 'P' : "",//"s",
      "METAL_QTY": this.salespersontargetform.controls.metal_division.value == "F" ? 'F' : "", //"s",
      "PROFIT": this.salespersontargetform.controls.diamond_division.value == "P" ? 'P' : "",//"s",
  ...
  calculate_total_diamond(){
    ...
    this.salespersontargetform.controls.diamondjewellery.setValue(this.commonService.decimalQuantityFormat(
    this.commonService.emptyToZero(total),"AMOUNT"));
  }
  ...
  calculate_total_qty(){
    ...
    this.salespersontargetform.controls.goldquantity.setValue(this.commonService.decimalQuantityFormat(
    this.commonService.emptyToZero(total),"METAL"));
  }
  

  addmonthtosubgrid(){
    let total_value = this.salespersontargetform.controls.diamondjewellery.value;
    let split_value= total_value/12;
    let months = [
  ...
  setdia_amount(){
    let total_value = this.salespersontargetform.controls.goldmakingcharge.value;
    let split_value= total_value/12;
    split_value = this.commonService.decimalQuantityFormat(
      this.commonService.emptyToZero(split_value),"AMOUNT");
    let datas = this.maindetails2;
    if (Array.isArray(datas)) {
      datas.forEach((e: any) => {
              ...
              let data = this.maindetails2;
                ...
                data.forEach((e: any) => {
                  e.GOLD_QTY = this.commonService.decimalQuantityFormat(
                    this.commonService.emptyToZero(0),"AMOUNT");
                });
              }
