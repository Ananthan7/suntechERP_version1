...
export class ZirconMasterComponent implements OnInit {
  ...
  zirconMasterMainFormSubmit() {
    Object.keys(this.zirconMasterMainForm.controls).forEach((controlName) => {
      const control = this.zirconMasterMainForm.controls[controlName];
  ...
  prefillPrefixCodes(event: any) {
    let code = event.target.value;
    this.getPrefixThroughCode(code);
  }

  getPrefixThroughCode(code: any) {
