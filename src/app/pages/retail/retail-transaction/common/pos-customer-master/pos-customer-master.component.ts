...
export class PosCustomerMasterComponent implements OnInit {
  ...
  changeMobileNumber(value: any = null) {
    ...
    if (_cust_mobile_no != "" && _cust_mobile_no != null) {
      ...
      // }
      this.apiService
        .getDynamicAPI(`PosCustomerMaster/GetCustomerMaster/${_cust_mobile_no}`)
        .subscribe((resp) => {
  ...
  getDropDownData() {
    this.maritalStatusList = this.comService
    ...
    this.genderList = this.comService
  ...
  onCountryCodeSelection(event: MatSelectChange, isCountrySelection: Boolean) {
    if (isCountrySelection) {
      const selectedOption = this.sortedCountryList.find(
        (item: any) => item.CODE === event.value
      );
      if (selectedOption) {
        this.customerDetailForm.controls["fcn_mob_code"].setValue(
      ...
      const selectedOption = this.sortedCountryList.find(
        (item: any) => item.MOBILECOUNTRYCODE === event.value
      );
      if (selectedOption) {
      ...
      if (!this.customerDetailForm.invalid) {
        const posCustomer = {
          ...
          ATTACHMENT_FROM_SCANNER:
            this.customerDetails?.ATTACHMENT_FROM_SCANNER || false,
          GOOD_QUALITY_A_K_A: "",
          LOW_QUALITY_A_K_A: "",
          POSKNOWNABOUT: 0,
        };
  ...
  getStateMasterByID(countryCode: any) {
    ...
    this.apiService.getDynamicAPI(API).subscribe(async (data) => {
      if (data.status == "Success") {
  ...
  getCityMasterByID(countryCode: any, stateCode: any) {
    let API = `GeneralMaster/GetGeneralMasterList/${encodeURIComponent(
      "CITY MASTER"
    )}/${encodeURIComponent(countryCode)}/${encodeURIComponent(stateCode)}`;
    this.apiService.getDynamicAPI(API).subscribe(async (data) => {
      if (data.status == "Success") {
