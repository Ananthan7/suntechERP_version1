...
export class GoldExchangeDetailsComponent implements OnInit {
  ...
  @ViewChild("overlayOutsideCode") overlayOutsideCode!: MasterSearchComponent;
  @ViewChild("overlayLocationCode") overlayLocationCode!: MasterSearchComponent;
  @Input() content!: any;
  ...
  ngOnInit(): void {
    ...
    else this.partyCurrency = this.partyCurrencyParam.partyCurrency;
  }
  ...
  stockCodeSelected(e: any) {
    ...
    this.goldExchangeDetailsForm.controls.stockCodeDescription.setValue(
      e.DESCRIPTION
    );
    ...
    let sub: Subscription = this.suntechApi
      ...
      .subscribe((res) => {
        ...
        const metalRate = this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.findKaratRate(_data.KARAT_CODE)),
          "RATE"
        );
        ...
        this.goldExchangeDetailsForm.controls.purity.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(_data.PURITY),
            "RATE"
          )
        );
      });
  ...
  updateStoneDiffrence() {
    ...
    this.goldExchangeDetailsForm.controls.stoneDiffer.setValue(
      this.comService.decimalQuantityFormat(
        this.comService.emptyToZero(stoneDiffrence),
        "METAL"
      )
    );
  }
    ...
    if (
      ...
      this.dialogBox.afterClosed().subscribe((data: any) => {
        if (data == "OK") {
          this.goldExchangeDetailsForm.controls.purity.setValue(
            this.comService.decimalQuantityFormat(
              this.comService.emptyToZero(this.standardPurity),
              "PURITY"
            )
          );
          ...
          this.goldExchangeDetailsForm.controls.pureWeight.setValue(
            this.comService.decimalQuantityFormat(
              this.comService.emptyToZero(this.standardPureWeight),
              "METAL"
            )
          );
      ...
      this.goldExchangeDetailsForm.controls.pureWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(
            this.goldExchangeDetailsForm.value.purity *
              this.goldExchangeDetailsForm.value.grossWeight
          ),
          "METAL"
        )
  ...
  updateStonePurityDiffrences() {
    const changedPurity =
      this.comService.emptyToZero(this.standardPureWeight) -
      this.comService.emptyToZero(
        this.goldExchangeDetailsForm.value.pureWeight
      );
    console.log(changedPurity);

    this.goldExchangeDetailsForm.controls.purityDiffer.setValue(
      this.comService.decimalQuantityFormat(
        this.comService.emptyToZero(changedPurity),
        "METAL"
      )
    );
  ...
  changeGrossweight(event: any) {
    ...
    this.goldExchangeDetailsForm.controls["grossWeight"].setValue(grossWt);

    const metalRate = this.goldExchangeDetailsForm.value.metalRate;

    const pureWt = this.comService.decimalQuantityFormat(
      this.comService.emptyToZero(
        this.goldExchangeDetailsForm.value.purity * event.target.value
      ),
      "METAL"
    );
    ...
    const metalAmount = this.comService.decimalQuantityFormat(
      this.comService.emptyToZero(
        (grossWt -
          this.comService.emptyToZero(
            this.goldExchangeDetailsForm.value.mudWeight
          )) *
          metalRate
      ),
      "AMOUNT"
    ...
    this.goldExchangeDetailsForm.controls.ozWeight.setValue(
      this.comService.decimalQuantityFormat(
        this.comService.emptyToZero(ozWeight),
        "AMOUNT"
      )
    );
  ...
  changeMudWeight(event: any) {
    ...
    if (
      ...
      this.dialogBox.afterClosed().subscribe((data: any) => {
        if (data == "OK") {
          this.goldExchangeDetailsForm.controls.mudWeight.setValue(
            this.comService.decimalQuantityFormat(
            ...
            this.comService.decimalQuantityFormat(
              this.comService.CCToFC(
                ...
                this.comService.emptyToZero(
                  ...
                  this.comService.emptyToZero(
                    this.goldExchangeDetailsForm.value.metalRate
                  )
              ),
              "AMOUNT"
            )
  ...
  updateNetTotal() {
    ...
    const netTotalAmount =
      ...
      this.comService.emptyToZero(
        this.goldExchangeDetailsForm.value.stoneAmount
      ) +
      this.comService.emptyToZero(
        this.goldExchangeDetailsForm.value.metalAmount
      );
  ...
  checkMetalRateLimit(event: any, object: string, metalRate: any) {
    ...
    if (object == "RATE") {
      if (
        this.comService.emptyToZero(event) >
        this.comService.emptyToZero(defaultMetalRate)
      ) {
        const baseMessage = this.comService.getMsgByID("MSG2203");
        this.openDialog("Warning", `${baseMessage}: ${defaultMetalRate}`, true);
  ...
  getKaratDetails() {
    this.suntechApi
      .getDynamicAPI("BranchKaratRate/" + this.comService.branchCode)
      .subscribe((result) => {
        if (result.response) {
          this.karatDetails = result.response;
          const defaultMetalRate = this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(this.findKaratRate("24")),
            "RATE"
          );
  ...
  findKaratRate(karatCode: string): number | undefined {
    const karatDetail = this.karatDetails.find(
      (detail) => detail.KARAT_CODE === karatCode
    );
  ...
  setExchangeDetails() {
    if (
      ...
      this.goldExchangeDetailsForm.controls.stockType.setValue(
        this.exchangeDetails.STOCK_CODE
      );
      this.goldExchangeDetailsForm.controls.fixMetalRate.setValue(
        this.exchangeDetails.CURRENCY_CODE
      );
      this.goldExchangeDetailsForm.controls.goldType.setValue(
        this.exchangeDetails.OLD_GOLD_TYPE
      );
      this.goldExchangeDetailsForm.controls.stockCodeDescription.setValue(
        this.exchangeDetails.STOCK_DOCDESC
      );
      this.goldExchangeDetailsForm.controls.locCode.setValue(
        this.exchangeDetails.LOCTYPE_CODE
      );
      ...
      this.goldExchangeDetailsForm.controls.grossWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.GROSSWT),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.stoneWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.STONEWT),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.purity.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.PURITY),
          "PURITY"
        )
      );
      this.goldExchangeDetailsForm.controls.pureWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.PUREWT),
          "METAL"
        )
      );
      ...
      this.goldExchangeDetailsForm.controls.mudWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.MUD_WT),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.netWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.NETWT),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.chargableWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.CHARGABLEWT),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.purityDiffer.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.PUDIFF),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.stoneDiffer.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.STONEDIFF),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.ozWeight.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.OZWT),
          "AMOUNT"
        )
      );

      this.goldExchangeDetailsForm.controls.unitValue.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.MKG_RATEFC),
          "AMOUNT"
        )
      );
      this.goldExchangeDetailsForm.controls.unitRate.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.MKGVALUEFC),
          "AMOUNT"
        )
      );
      this.goldExchangeDetailsForm.controls.unitAmount.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.MKGVALUECC),
          "AMOUNT"
        )
      );
      this.goldExchangeDetailsForm.controls.stoneRate.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.STONEVALUEFC),
          "AMOUNT"
        )
      );
      ...
      this.goldExchangeDetailsForm.controls.stoneAmount.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.STONEVALUECC),
          "AMOUNT"
        )
      );
      this.goldExchangeDetailsForm.controls.wastagePercent.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.WASTAGEPER),
          "AMOUNT"
        )
      );
      this.goldExchangeDetailsForm.controls.wastageQuantity.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.WASTAGEQTY),
          "METAL"
        )
      );
      this.goldExchangeDetailsForm.controls.wastageAmount.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.WASTAGEAMOUNTFC),
          "AMOUNT"
        )
      );
      ...
      this.goldExchangeDetailsForm.controls.metalRate.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.METALVALUEFC),
          "RATE"
        )
      );
      this.goldExchangeDetailsForm.controls.metalAmount.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.METALVALUECC),
          "AMOUNT"
        )
      );
      this.goldExchangeDetailsForm.controls.netRate.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.NETVALUEFC),
          "AMOUNT"
        )
      );
      this.goldExchangeDetailsForm.controls.netAmount.setValue(
        this.comService.decimalQuantityFormat(
          this.comService.emptyToZero(this.exchangeDetails.NETVALUECC),
          "AMOUNT"
        )
      );

      this.goldExchangeDetailsForm.controls.remarks.setValue(
        this.exchangeDetails.D_REMARKS
      );
      this.goldExchangeDetailsForm.controls.bagNo.setValue(
        this.exchangeDetails.BAGNO
      );
  ...
  setFormValues() {
    ...
    this.goldExchangeDetailsForm.controls.grossWeight.setValue(
      this.content.GROSSWT
    );
    this.goldExchangeDetailsForm.controls.stoneWeight.setValue(
      this.content.STONEWT
    );
    this.goldExchangeDetailsForm.controls.netWeight.setValue(
      this.content.NETWT
    );
    ...
    this.goldExchangeDetailsForm.controls.chargableWeight.setValue(
      this.content.CHARGABLEWT
    );
    this.goldExchangeDetailsForm.controls.unitRate.setValue(
      this.content.MKGVALUEFC
    );
    this.goldExchangeDetailsForm.controls.unitAmount.setValue(
      this.content.MKGVALUECC
    );
    this.goldExchangeDetailsForm.controls.metalRate.setValue(
      this.content.METALVALUEFC
    );
    this.goldExchangeDetailsForm.controls.metalAmount.setValue(
      this.content.METALVALUECC
    );
    this.goldExchangeDetailsForm.controls.stoneRate.setValue(
      this.content.STONEVALUEFC
    );
    this.goldExchangeDetailsForm.controls.stoneAmount.setValue(
      this.content.STONEVALUECC
    );
    this.goldExchangeDetailsForm.controls.netRate.setValue(
      this.content.NETVALUEFC
    );
    this.goldExchangeDetailsForm.controls.netAmount.setValue(
      this.content.NETVALUECC
    );
    this.goldExchangeDetailsForm.controls.purityDiffer.setValue(
      this.content.PUDIFF
    );
    this.goldExchangeDetailsForm.controls.stoneDiffer.setValue(
      this.content.STONEDIFF
    );
    this.goldExchangeDetailsForm.controls.locCode.setValue(
      this.content.LOCTYPE_CODE
    );
    this.goldExchangeDetailsForm.controls.ozWeight.setValue(this.content.OZWT);
    this.goldExchangeDetailsForm.controls.supplier.setValue(
      this.content.SUPPLIER
    );
    this.goldExchangeDetailsForm.controls.stockCodeDescription.setValue(
      this.content.STOCK_DOCDESC
    );
    this.goldExchangeDetailsForm.controls.wastagePercent.setValue(
      this.content.WASTAGEPER
    );
    this.goldExchangeDetailsForm.controls.wastageQuantity.setValue(
      this.content.WASTAGEQTY
    );
    this.goldExchangeDetailsForm.controls.wastageAmount.setValue(
      this.content.WASTAGEAMOUNTFC
    );
    this.goldExchangeDetailsForm.controls.goldType.setValue(
      this.content.OLD_GOLD_TYPE
    );
    this.goldExchangeDetailsForm.controls.outsideGold.setValue(
      this.content.OUTSIDEGOLD
    );
    this.goldExchangeDetailsForm.controls.jawaharaSelect.setValue(
      this.content.JAWAHARAYN
    );
    this.goldExchangeDetailsForm.controls.reSaleCycleSelect.setValue(
      this.content.RESALERECYCLE
    );
    this.goldExchangeDetailsForm.controls.cashExchangeSelect.setValue(
      this.content.CASHEXCHANGE
    );
    this.goldExchangeDetailsForm.controls.remarks.setValue(
      this.content.D_REMARKS
    );
  }
  ...
  update() {
    ...
    const updateApi =
      "OldGoldPurchase/UpdateMetalPurchase/" +
      this.branchCode +
      "/" +
  ...
  openPanel(event: any, formControlName: string) {
    switch (formControlName) {
      case "stockCode":
        ...
        break;
      case "goldType":
        this.overlayOutsideCode.showOverlayPanel(event);
        break;
      case "locCode":
