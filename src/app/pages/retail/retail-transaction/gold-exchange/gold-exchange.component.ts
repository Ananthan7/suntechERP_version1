...
export class GoldExchangeComponent implements OnInit {
  ...
  setCompanyCurrencies() {
    ...
    this.goldExchangeForm.controls.rndOfAmt.setValue(
      this.comService.compCurrency
    );
    this.goldExchangeForm.controls.rndNetAmt.setValue(
      this.comService.compCurrency
    );
    this.goldExchangeForm.controls.otherAmt.setValue(
      this.comService.compCurrency
    );
    this.goldExchangeForm.controls.grossAmt.setValue(
      this.comService.compCurrency
    );
  }
          ...
          if (result.status == "Success") {
            ...
            if (result.dynamicData.length > 0) {
              ...
              if (data && data[0].CURRENCY_CODE) {
                ...
                this.goldExchangeForm.controls.itemCurr.setValue(
                  data[0].CURRENCY_CODE
                );
  ...
  async getFinancialYear() {
    const API = `BaseFinanceYear/GetBaseFinancialYear/${this.comService.cDateFormat(
      this.goldExchangeForm.value.vocDate
    )}`;
  ...
  handlePostData(postData: any) {
    const preItemIndex = this.goldExchangeDetailsData.findIndex(
    ...
    console.log(
      "Updated goldExchangeDetailsData",
      this.goldExchangeDetailsData
    );
  ...
  getRetailSalesMaster(data: any) {
    ...
    this.suntechApi.getDynamicAPI(API).subscribe((res) => {
      if (res.status == "Success") {
        ...
        this.goldExchangeForm.controls.amount.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.TOTSTAMP_AMTFC),
            "AMOUNT"
          )
        );
        this.goldExchangeForm.controls.amountDes.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.TOTSTAMP_AMTCC),
            "AMOUNT"
          )
        );
        this.goldExchangeForm.controls.rndOfAmt.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.ITEM_VALUE_FC),
            "AMOUNT"
          )
        );
        this.goldExchangeForm.controls.rndOfAmtDes.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.ITEM_VALUE_CC),
            "AMOUNT"
          )
        );
        ...
        this.goldExchangeForm.controls.rndNetAmtDes.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.NET_VALUE_CC),
            "AMOUNT"
          )
        );
        this.goldExchangeForm.controls.otherAmt.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.ADDL_VALUE_FC),
            "AMOUNT"
          )
        );
        this.goldExchangeForm.controls.otherAmtDes.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.ADDL_VALUE_CC),
            "AMOUNT"
          )
        );
        this.goldExchangeForm.controls.grossAmt.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.GROSS_VALUE_FC),
            "AMOUNT"
          )
        );
        this.goldExchangeForm.controls.grossAmtDes.setValue(
          this.comService.decimalQuantityFormat(
            this.comService.emptyToZero(data.GROSS_VALUE_CC),
            "AMOUNT"
          )
        );
  ...
  setFormValues() {
    ...
    this.goldExchangeForm.controls.partyCurrCodeDesc.setValue(
      this.content.PARTY_CURR_RATE
    );
    this.goldExchangeForm.controls.itemCurr.setValue(
      this.content.ITEM_CURRENCY
    );
    this.goldExchangeForm.controls.itemCurrCode.setValue(
      this.content.ITEM_CURR_RATE
    );
    this.goldExchangeForm.controls.salesMan.setValue(
      this.content.SALESPERSON_CODE
    );
    this.goldExchangeForm.controls.partyCurrency.setValue(
      this.content.PARTY_VALUE_FC
    );
    this.goldExchangeForm.controls.partyCurrencyCode.setValue(
      this.content.PARTY_VALUE_CC
    );
    this.goldExchangeForm.controls.partyCurrencyCode.setValue(
      this.content.PARTY_VALUE_CC
    );
    this.goldExchangeForm.controls.rndNetAmt.setValue(
      this.content.NET_VALUE_FC
    );
    ...
    this.goldExchangeForm.controls.rndNetAmtDes.setValue(
      this.content.NET_VALUE_CC
    );
    this.goldExchangeForm.controls.otherAmt.setValue(
      this.content.ADDL_VALUE_FC
    );
    this.goldExchangeForm.controls.otherAmtDes.setValue(
      this.content.ADDL_VALUE_CC
    );
    this.goldExchangeForm.controls.grossAmt.setValue(
      this.content.GROSS_VALUE_FC
    );
    this.goldExchangeForm.controls.grossAmtDes.setValue(
      this.content.GROSS_VALUE_CC
    );
    ...
    this.goldExchangeForm.controls.creditDaysCode.setValue(
      this.content.CREDITDAY
    );

    this.goldExchangeForm.controls.customer.setValue(
      this.content.HLOCTYPE_CODE
    );
    ...
    this.goldExchangeForm.controls.moblie.setValue(
      this.content.CUSTOMER_MOBILE
    );
    ...
    this.goldExchangeForm.controls.amountDes.setValue(
      this.content.TOTSTAMP_AMTCC
    );
    this.goldExchangeForm.controls.rndOfAmt.setValue(
      this.content.VATAMOUNTFCROUND
    );
    this.goldExchangeForm.controls.rndOfAmtDes.setValue(
      this.content.VATAMOUNTFCROUNDCC
    );
  }
  ...
  formSubmit() {
    ...
    let postData = {
      ...
      TOTAL_PUDIFF: 0,
      TOTAL_STONEDIFF: 0,
      ITEM_VALUE_FC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.rndOfAmt,
        "AMOUNT"
      ),
      ITEM_VALUE_CC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.rndOfAmtDes,
        "AMOUNT"
      ),
      PARTY_VALUE_FC: 0,
      PARTY_VALUE_CC: 0,
      NET_VALUE_FC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.rndNetAmt,
        "AMOUNT"
      ),
      NET_VALUE_CC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.rndNetAmtDes,
        "AMOUNT"
      ),
      ADDL_VALUE_FC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.otherAmt,
        "AMOUNT"
      ),
      ADDL_VALUE_CC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.otherAmtDes,
        "AMOUNT"
      ),
      GROSS_VALUE_FC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.grossAmt,
        "AMOUNT"
      ),
      GROSS_VALUE_CC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.grossAmtDes,
        "AMOUNT"
      ),
      ...
      SHIPDESC: "",
      STAMPCHARGE: true,
      TOTSTAMP_AMTFC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.amount,
        "AMOUNT"
      ),
      TOTSTAMP_AMTCC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.amountDes,
        "AMOUNT"
      ),
      ...
      TRANSFER_BRANCH: "",
      VATAMOUNTFCROUND: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.rndOfAmt,
        "AMOUNT"
      ),
      VATAMOUNTFCROUNDCC: this.comService.decimalQuantityFormat(
        this.goldExchangeForm.value.rndOfAmtDes,
        "AMOUNT"
      ),
    ...
    let Sub: Subscription = this.suntechApi
      .postDynamicAPI(saveApi, postData)
      .subscribe(
        (result) => {
          if (result.response) {
            if (result.status == "Success") {
  ...
  update() {
    ...
    const updateApi =
      ...
      "/" +
      this.goldExchangeForm.value.vocTypeNo +
      "/" +
      this.yearMonth;
  ...
  creditDate(event: any) {
    ...
    this.goldExchangeForm.patchValue({
      creditDays: newDate,
    });
  }
}
