...
export class SchemeRegisterComponent implements OnInit {
  ...
  openAttchments(e?: any) {
    ...
    let API = `SchemeRegistration/GetSchemeAttachments/`+this.content?.SCH_CUSTOMER_ID.toString()
  ...
  processSchemeAPI() {
    ...
    API += this.commonService.branchCode + '/'
    API += this.commonService.nullToString(formValue.SchemeId) + '/'
    API += this.commonService.nullToString(joindate) + '/'
    API += this.commonService.emptyToZero(formValue.AlertBeforeDays) + '/'
    API += this.commonService.emptyToZero(formValue.TenurePeriod) 
    let sub: Subscription = this.dataService.getDynamicAPI(API)
      .subscribe((resp: any) => {
        if (resp) {
          this.viewMode = true;
  ...
  searchValueChange(event: any, searchFlag: string, schemeFlag?: boolean) {
    ...
    let API = `PosCustomerMaster/GetCustomerByCode/${event.target.value}`
