...
export class AddSchemeComponent implements OnInit {
  ...
  SchemeMasterFindData: any = {
    API_VALUE: 'Scheme/SchemeMasterLookUp',
  ...
  setInitialValues(data: any) {
    if (data) {
      ...
      if (data.SCHEME_UNIQUEID) {
        this.receiptEntryForm.controls.SchemeUniqueNo.setValue(data.SCHEME_UNIQUEID)
      }
      if (data.SCHEME_CUSTCODE) {
        this.receiptEntryForm.controls.SCHEME_CUSTCODE.setValue(data.SCHEME_CUSTCODE)
      }
      ...
      this.receiptEntryForm.controls.TotalValue.setValue(data.SCHEME_TOTAL_VALUE)
      ...
      let datediff = this.getMonthDifference(new Date(data.SCHEME_STARTED),new Date(data.SCHEME_ENDEDON))
    ...
    let Sub: Subscription = this.dataService.getDynamicAPI('Scheme/SalesPersonMaster')
      .subscribe((result) => {
        if (result.response) {
          ...
          this.filteredOptions = this.receiptEntryForm.controls.Salesman.valueChanges.pipe(
            startWith(''),
  ...
  schemeUnitChanged(data: any) {
    this.receiptEntryForm.controls.TotalValue.setValue(this.receiptEntryForm.value.SchemeAmount*data.target.value)
  }
  ...
  branchChange(event: any) {
    ...
    let data = this.branchArray.filter((item: any) => item.BRANCH_CODE == inputValue)
    if (data.length > 0) {
  ...
  salesmanChange(event: any) {
    ...
    if (data.length > 0) {
      this.receiptEntryForm.controls.Salesman.setValue(data[0].SALESPERSON_CODE)
    } else {
