...
export class RepairDiamondPurchaseComponent implements OnInit {
  ...
  credit_Date(event: any) {
    const credit_days_LC = +event.target.value;
    const newDate = new Date(this.currentDate);
    newDate.setDate(newDate.getDate() + credit_days_LC);

    this.repairdiapurchaseForm.patchValue({
      creditDate: newDate,
    });
  }
