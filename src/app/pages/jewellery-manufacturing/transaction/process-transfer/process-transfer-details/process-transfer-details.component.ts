...
export class ProcessTransferDetailsComponent implements OnInit {
      ...
      .subscribe((result) => {
        ...
        if (result.status == "Success" && result.dynamicData[0]) {
          ...
          if (data[0] && data[0].MSG_ID == '') {
            ...
            this.comService.toastErrorByMsgId(data[0].MSG_ID)
            return
          }
        } else {
  ...
  formSubmit() {
    ...
    detailDataSet.PROCESSFORMDETAILS = this.processTransferdetailsForm.value;
    this.close([detailDataSet])//USE: passing Detail data to header screen on close
