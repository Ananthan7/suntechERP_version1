...
export class CostCentreDiamondComponent implements OnInit {
  ...
  columnhead:any[] = [  { title:'Division' , field: 'DIVISION_CODE'}, { title:'Description', field: 'DESCRIPTION'}];
  ...
  ngOnInit(): void {
    const apiUrl = '/CostCenterMaster/GetCostCenterDivCodeAndDescGrid';  
      let sub: Subscription = this.dataService.getDynamicAPI(apiUrl).subscribe((resp: any) => {
        if (resp.status == 'Success') {
          this.tableDataDivision = resp.response;
          console.log(this.tableDataDivision);
        }       
      });
  }
